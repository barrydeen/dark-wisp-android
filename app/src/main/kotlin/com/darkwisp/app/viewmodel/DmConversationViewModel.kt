...
import com.darkwisp.app.nostr.SignerCancelledException
...
class DmConversationViewModel(app: Application) : AndroidViewModel(app) {
    ...
    fun decryptPending(
        ...
        eventRepo: com.darkwisp.app.repo.EventRepository? = null,
        notifRepo: com.darkwisp.app.repo.NotificationRepository? = null
    ) {
                    ...
                    try {
                        ...
                        if (Nip17.isReaction(rumor)) {
                            ...
                            val emojiUrl = if (emojiContent.startsWith(":") && emojiContent.endsWith(":")) {
                                com.darkwisp.app.nostr.Nip30.parseEmojiTags(rumor.tags)[emojiContent.removeSurrounding(":")]
                            } else null
                        ...
                        val emojiMap = com.darkwisp.app.nostr.Nip30.parseEmojiTags(rumor.tags)
                        val fileMetadata = if (Nip17.isFileMessage(rumor)) {
                            com.darkwisp.app.nostr.EncryptedMedia.parseKind15Tags(rumor.tags, rumor.content)
                        } else null
                        val msg = DmMessage(
    ...
    private suspend fun fetchRecipientDmRelays(relayPool: RelayPool, forceRefresh: Boolean = false): List<String> {
        ...
        return com.darkwisp.app.repo.DmRelayLookup.fetch(peerPubkey, relayPool, repo, forceRefresh)
    }
    ...
    fun sendMessage(relayPool: RelayPool, signer: NostrSigner? = null, resolvedEmojis: Map<String, String> = emptyMap()) {
        ...
        if (signer != null) {
            ...
            viewModelScope.launch(Dispatchers.Default) {
                try {
                    ...
                    val emojiTags = com.darkwisp.app.nostr.Nip30.buildEmojiTagsForContent(text, resolvedEmojis)
                    ...
                    val sentEmojiMap = com.darkwisp.app.nostr.Nip30.buildEmojiTagsForContent(text, resolvedEmojis)
        ...
        viewModelScope.launch(Dispatchers.Default) {
            try {
                val senderPubkeyHex = keypair.pubkey.toHex()
                val emojiTags2 = com.darkwisp.app.nostr.Nip30.buildEmojiTagsForContent(text, resolvedEmojis)
